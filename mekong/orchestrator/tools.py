...
def write_file(repo_root: Path, rel_path: str, body: str) -> ToolResult:
    ...
    if any(rel_str.startswith(p) for p in forbidden) and rel_str not in (
        "apps/dashboard/.env.local.example",
    ):
        return ToolResult(
            False,
            f"REJECTED: writes to {rel_path} are blocked by CLAUDE.md public-repo boundary. Use --override-boundary if absolutely intentional.",
        )
...
def edit_file(repo_root: Path, rel_path: str, body: str) -> ToolResult:
    ...
    if text.count(old) > 1:
        return ToolResult(
            False, f"<old> block matches {text.count(old)}× in {rel_path} — make it more specific"
        )
