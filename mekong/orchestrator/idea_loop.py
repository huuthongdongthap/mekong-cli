...
class IdeaLoop:
    ...
    def _maybe_compact(self) -> None:
        ...
        self.ctx.add(
            "user",
            "Context near limit. Summarize all prior turns into ≤2k tokens covering: plan, tasks done, files touched, current issue, next step.",
        )
    ...
    def run(self) -> RunResult:
        ...
        while True:
            ...
            if not call:
                self.ctx.add(
                    "user",
                    "ERROR: no <tool>...</tool> block detected. Re-emit your last decision wrapped in a tool call.",
                )
