...
def infer_tier(spec_path: str) -> str:
    ...
    if any(
        x in name
        for x in ["annual", "okr", "fundraise", "swot", "pitch", "ipo", "vc", "board", "cap-table"]
    ):
        return "founder"
    if any(
        x in name
    ...
    if any(
        x in name
        for x in ["plan", "sprint", "roadmap", "brainstorm", "scope", "product", "prd", "backlog"]
    ):
        return "product"
    if any(
        x in name
...
def write_live_md(rows: list, out_path: str, audit_date: str) -> None:
    ...
    lines = [
        "# Live Commands — Verified Executable",
        "",
        f"> Audit date: {audit_date}",
        "> Definition: Python module exists in `src/commands/` with matching spec name.",
        "",
        ...
        "",
        "| # | Command | Spec Path | Module Path | Tier |",
        "|---|---------|-----------|-------------|------|",
    ]
...
def write_audit_index_md(data: dict, out_path: str, audit_date: str) -> None:
    ...
    lines = [
        ...
        "## Links",
        "",
        "- [Audit README](docs/command-audit/README.md)",
        "- [Live Commands](docs/command-audit/live-commands.md)",
        "- [Spec-Only Commands](docs/command-audit/spec-only-commands.md)",
        "- [Ghost Commands](docs/command-audit/ghost-commands.md)",
        "- [Mapping Table CSV](docs/command-audit/mapping-table.csv)",
    ]
    with open(out_path, "w") as f:
        f.write("\n".join(lines) + "\n")
