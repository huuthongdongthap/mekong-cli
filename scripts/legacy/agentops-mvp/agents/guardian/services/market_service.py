...
class MarketService:
    ...
    def analyze_market_position(self, terms: Dict[str, Any], stage: str) -> Dict[str, Any]:
        ...
        if liq_pref == benchmark["liq_pref_typical"]:
            comparisons.append(
                {"term": "Liquidation Preference", "status": "MARKET", "benchmark": "1x"}
            )
        elif liq_pref > benchmark["liq_pref_typical"]:
            comparisons.append(
                {"term": "Liquidation Preference", "status": "UNFAVORABLE", "benchmark": "1x"}
            )
        ...
        return {
            ...
            "overall": (
                "FAVORABLE"
                if all(c["status"] in ["MARKET", "FAVORABLE"] for c in comparisons)
                else "NEEDS_NEGOTIATION"
            ),
