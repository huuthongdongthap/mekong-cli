...
def scan_secrets():
            ...
            if file.endswith(".py"):
                ...
                try:
                    ...
                    for pattern, secret_type in patterns:
                        ...
                        if matches:
                            findings.append(
                                {"file": str(filepath), "type": secret_type, "count": len(matches)}
                            )
                except Exception:
                    pass
    ...
    for env_file in env_files:
        findings.append(
            {"file": str(env_file), "type": "Environment File", "status": "Should be in .gitignore"}
        )
...
def scan_usage_metering():
    ...
    if checks["usage_tracker"]:
        content = (
            checks["usage_tracker"].read_text()
            if hasattr(checks["usage_tracker"], "read_text")
            else Path("src/usage/usage_tracker.py").read_text()
        )
    ...
    if checks["usage_metering_service"]:
        ...
        checks["circuit_breaker"] = (
            "circuit_breaker" in content.lower() or "CircuitBreaker" in content
        )
...
def scan_test_coverage():
    ...
    roi_tests = [
        ...
        if any(
            x in f.name
            for x in ["license", "usage", "webhook", "command", "tracing", "sbom", "validation"]
        )
    ]
