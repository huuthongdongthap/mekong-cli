    ...
    try:
        while not shutdown_requested:
            ...
            if daemon_proc.stdout:
                ...
                if line:
                    _write_event(log_fh, "daemon_stdout", msg=_redact(line.rstrip()))

            # Check if daemon died — restart it
            if daemon_proc.poll() is not None:
                exit_code = daemon_proc.returncode
                _write_event(
                    log_fh, "daemon_exit", exit_code=exit_code, restart_count=restart_count
                )
                ...
                restart_count += 1
                _write_event(
                    log_fh, "daemon_restart", new_pid=daemon_proc.pid, restart_count=restart_count
                )
        ...
        elapsed = time.monotonic() - start_time
        _write_event(log_fh, "harness_end", uptime_s=int(elapsed), restart_count=restart_count)
        log_fh.close()
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Use dummy daemon (no real LLM calls)"
    )
