...
class TestHealthWatchdog:
    ...
    def test_evaluate_all_clear(self):
        wd = HealthWatchdog()
        wd.register(HealthCheck(name="low", description="Low metric", check_fn=lambda _: 10.0))
        report = wd.evaluate()
    ...
    def test_evaluate_error_treated_as_critical(self):
        ...
        wd = HealthWatchdog()
        wd.register(HealthCheck(name="error", description="Error check", check_fn=bad_check))
        report = wd.evaluate()
        assert report.criticals == 1
    ...
    def test_state_tracking(self):
        wd = HealthWatchdog()
        wd.register(
            HealthCheck(name="s", description="t", check_fn=lambda _: 85.0, warning_threshold=80)
        )
        wd.evaluate()
