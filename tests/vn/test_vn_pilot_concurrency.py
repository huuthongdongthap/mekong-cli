...
class TestAppendConcurrency:
    ...
    def test_no_corruption_under_8_concurrent_writers(self, tmp_path: Path) -> None:
    ...
    def test_repeated_runs_stable(self, tmp_path: Path) -> None:
        ...
        for run in range(3):
            ...
            if pilots_file.exists():
                pilots_file.unlink()
            processes = [
                ctx.Process(target=_writer_worker, args=(wid, config_dir, 5)) for wid in range(4)
            ]
            for p in processes:
                p.start()
            for p in processes:
