...
def _insert_pilot(
    db_path: Path, user_id: str, onboarded_at: str = "2026-01-01T00:00:00.000000Z"
) -> None:
...
class TestFreshSeed:
    def test_fresh_seed_creates_default_org_and_backfills(self, tmp_path: Path) -> None:
    ...
    def test_empty_pilots_table_seeds_only_default_org_and_founder(self, tmp_path: Path) -> None:
...
class TestPrereqs:
    def test_missing_storage_env_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestReservedSlug:
    def test_reserved_slug_still_rejected_post_seed(self, monkeypatch: pytest.MonkeyPatch) -> None:
