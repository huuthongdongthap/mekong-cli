...
class TestSignupParity:
    def test_signup_returns_201_is_new(self, client_backend: tuple[TestClient, str]) -> None:
...
class TestConvertRevenueParity:
    def test_revenue_increments_mrr(self, client_backend: tuple[TestClient, str]) -> None:
        client, backend = client_backend
        # First signup
    ...
    def test_convert_idempotent(self, client_backend: tuple[TestClient, str]) -> None:
        client, backend = client_backend
        signup_resp = client.post("/v1/pilot/signup", json=VALID_SIGNUP)
        user_id = signup_resp.json()["user_id"]
...
class TestPollParity:
    def test_poll_response_recorded(self, client_backend: tuple[TestClient, str]) -> None:
        client, backend = client_backend
        signup_resp = client.post("/v1/pilot/signup", json=VALID_SIGNUP)
        user_id = signup_resp.json()["user_id"]
...
class TestOrgIsolationParity:
    def test_cross_tenant_isolation(self, client_backend: tuple[TestClient, str]) -> None:
...
class TestHealthStatsParity:
    ...
    def test_stats_counts_after_signup(self, client_backend: tuple[TestClient, str]) -> None:
        client, backend = client_backend
