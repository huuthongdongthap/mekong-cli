...
def test_first_member_auto_added_as_org_admin(tmp_path: Path) -> None:
    ...
    rows = conn.execute("SELECT * FROM org_members WHERE org_id = ?", (org_id,)).fetchall()
    conn.close()
...
def test_mint_jwt_integration_org_admin(tmp_path: Path) -> None:
    ...
    assert (
        org_id in claims["allowed_orgs"]
    ), f"Expected org_id '{org_id}' in allowed_orgs={claims['allowed_orgs']}"
    assert "org_admin" in claims["scopes"], f"Expected 'org_admin' in scopes={claims['scopes']}"
