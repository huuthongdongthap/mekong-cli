...
class TestConversionIdempotency:
    ...
    def test_unique_user_started_at(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        backend = _backend_for(tmp_path, monkeypatch)
    ...
    def test_unique_bank_tx_ref(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        backend = _backend_for(tmp_path, monkeypatch)
        self._add_pilot(backend)
        rec = {
            "user_id": "opc_001_abc",
...
class TestPollResponseIdempotency:
    def test_unique_user_iso_week(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        backend = _backend_for(tmp_path, monkeypatch)
        rec = {
            "user_id": "opc_001_abc",
            "score": 4,
...
class TestCredits:
    ...
    def test_add_credits_basic(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        backend = _backend_for(tmp_path, monkeypatch)
