...
class TestJWTFounderScope:
    def test_founder_scope_convert(self, client: TestClient) -> None:
...
class TestBothSecretsAbsent:
    def test_503_when_both_missing(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestAuditLog:
    def test_audit_log_emitted_on_jwt_success(
        ...
        audit_lines = [r.message for r in caplog.records if '"event": "admin_auth"' in r.message]
