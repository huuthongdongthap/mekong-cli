...
def _seed_org(display_name: str = "Acme Corp", founder: str = "founder@acme.vn") -> dict:
...
# =============================================================================
def test_create_invite_idempotent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_accept_expired_invite_returns_410(
    ...
    past = (datetime.now(timezone.utc) - timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S.%f") + "Z"
    conn = _db_conn(tmp_path)
    conn.execute(
