...
def app(monkeypatch) -> FastAPI:
    ...
    async def protected(request: Request, tenant_id: str = Depends(license_gate)) -> dict:
...
class TestLicenseStateChecks:
    ...
    def test_success_path_returns_tenant(self, app, jwt_secret, _isolate_license_store):
