...
class TestHarnessSmoke:
    ...
    def test_log_has_harness_start_event(self, smoke_run):
        ...
        assert (
            start_events
        ), f"No harness_start event. Events: {[e.get('event') for e in events[:5]]}"
    ...
    def test_at_least_one_daemon_stdout_event(self, smoke_run):
        ...
        content = log_path.read_text()
        assert (
            "daemon_stdout" in content or "daemon_start" in content
    ...
    def test_elapsed_within_expected_range(self, smoke_run):
        ...
        assert elapsed < SMOKE_DURATION + 30, f"Run took too long ({elapsed:.1f}s) — possible hang"
