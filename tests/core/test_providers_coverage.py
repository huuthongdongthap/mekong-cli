...
class TestGeminiProviderChat:
    ...
    def test_chat_success(self):
        ...
        result = p.chat([{"role": "user", "content": "hello"}], "gemini-pro", 0.7, 200, False)
    ...
    def test_chat_with_json_mode(self):
        ...
        p.chat([{"role": "user", "content": "return json"}], "gemini-pro", 0.0, 100, True)
        config_arg = p._client.models.generate_content.call_args[1]["config"]
    ...
    def test_chat_text_via_candidates_fallback(self):
        ...
        result = p.chat([{"role": "user", "content": "something"}], "gemini-pro", 0.5, 100, False)
    ...
    def test_chat_retryable_error_retries(self):
        ...
        with patch("time.sleep"):
            result = p.chat([{"role": "user", "content": "hi"}], "gemini-pro", 0.5, 100, False)
...
class TestOpenAICompatibleProvider:
    ...
    def test_name_uses_provider_name(self):
        p = OpenAICompatibleProvider(base_url="http://x.com", provider_name="my-provider")
    ...
    def test_chat_success(self):
        ...
        with (
            patch("src.core.model_alias.resolve_model", return_value="gpt-4"),
            patch("urllib.request.urlopen", return_value=mock_resp),
        ):
            result = p.chat([{"role": "user", "content": "hi"}], "gpt-4", 0.7, 100, False)
    ...
    def test_chat_with_api_key_adds_auth_header(self):
        ...
        p = OpenAICompatibleProvider(base_url="http://localhost:1234", api_key="sk-test", model="m")
        response_data = {
        ...
        with (
            patch("src.core.model_alias.resolve_model", return_value="m"),
...
class TestOfflineProvider:
    ...
    def test_chat_returns_offline_message(self):
        p = OfflineProvider()
        result = p.chat([{"role": "user", "content": "hello world"}], "any", 0.5, 100, False)
    ...
    def test_chat_no_user_message_uses_unknown(self):
        p = OfflineProvider()
        result = p.chat([{"role": "system", "content": "be helpful"}], "any", 0.5, 100, False)
    ...
    def test_chat_truncates_long_messages(self):
        p = OfflineProvider()
        long_msg = "x" * 300
        result = p.chat([{"role": "user", "content": long_msg}], "any", 0.5, 100, False)
...
class TestLiteLLMProvider:
    ...
    def test_chat_success(self):
        ...
        with patch.dict("sys.modules", {"httpx": mock_httpx}):
            p = LiteLLMProvider(base_url="http://localhost:4000")
            p._available = True
            result = p.chat([{"role": "user", "content": "hi"}], "gpt-4", 0.5, 100, False)
