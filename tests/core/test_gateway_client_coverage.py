...
def _make_client(local_test=False) -> GatewayClient:
    ...
    with (
        patch("src.core.gateway_client.get_auth_client", return_value=mock_auth),
...
class TestRequestLive:
    ...
    def test_200_returns_gateway_response(self):
        ...
        session.request.return_value = _make_response(
            200, {"result": "ok"}, {"X-RateLimit-Remaining": "50"}
        )
        client = self._patched_client(session)
        client.auth._load_credentials.return_value = {}

        with patch.object(
            client, "_get_auth_header", return_value=({"Content-Type": "application/json"}, None)
        ):
    ...
    def test_all_circuits_open_uses_primary(self):
        ...
        with (
            patch.object(client, "_get_available_gateway", return_value=None),
            patch.object(client, "_get_auth_header", return_value=({}, None)),
        ):
            response = client.request("GET", "/v1/health")
...
class TestHealthCheck:
    def test_health_check_success(self):
        client = _make_client()
        mock_resp = GatewayResponse(status_code=200, data={}, headers={}, elapsed_ms=1.0)
...
class TestGetCircuitStatus:
    ...
    def test_circuit_status_open(self):
        client = _make_client()
        url = "https://test.url"
        client._circuit_states = {
            url: CircuitState(circuit_open=True, failure_count=3, last_failure_time=time.time())
        }
        status = client.get_circuit_status()
...
class TestGetGatewayClientSingleton:
    def test_returns_same_instance(self):
        ...
        with (
            patch("src.core.gateway_client.get_auth_client", return_value=MagicMock()),
