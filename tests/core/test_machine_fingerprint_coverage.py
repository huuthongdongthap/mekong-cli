...
class TestGetMacAddresses(unittest.TestCase):
    ...
    def test_windows_dispatch(self):
        ...
        with patch.object(
            gen, "_get_mac_addresses_windows", return_value=["aa:aa:aa:aa:aa:aa"]
        ) as m:
            gen._get_mac_addresses()
            m.assert_called_once()
    ...
    def test_loopback_filtered_out(self):
        ...
        gen.platform = "Linux"
        with patch.object(
            gen, "_get_mac_addresses_linux", return_value=["00:00:00:00:00:00", "aa:bb:cc:dd:ee:ff"]
        ):
            macs = gen._get_mac_addresses()
    ...
    def test_duplicate_macs_deduplicated(self):
        ...
        gen.platform = "Linux"
        with patch.object(
            gen, "_get_mac_addresses_linux", return_value=["aa:bb:cc:dd:ee:ff", "aa:bb:cc:dd:ee:ff"]
        ):
            macs = gen._get_mac_addresses()
    ...
    def test_macs_are_sorted(self):
        ...
        gen.platform = "Linux"
        with patch.object(
            gen, "_get_mac_addresses_linux", return_value=["cc:cc:cc:cc:cc:cc", "aa:aa:aa:aa:aa:aa"]
        ):
            macs = gen._get_mac_addresses()
        ...
        with self._patch_windows_env():
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(
                    returncode=0, stdout="AA-BB-CC-DD-EE-FF\n", stderr=""
                )
...
class TestGetMachineIdWindows(unittest.TestCase):
    ...
    def test_powershell_returns_guid(self):
        ...
        with self._patch_windows_env():
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(
                    returncode=0, stdout="12345678-abcd-efab-cdef-000000000001\n", stderr=""
                )
