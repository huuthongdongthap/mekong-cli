...
def clear_env_overrides(monkeypatch):
    """Ensure environment overrides like LLM_MODEL don't interfere with routing matrix tests."""
...
class TestSelectModel:
    ...
    def test_starter_prefers_local_non_critical(self):
        profile = _make_profile(
            agent_role="coo", complexity="simple", requires_reasoning=False, domain="ops"
        )
        state = _make_state(tenant_tier="starter")
        config = select_model(profile, state)
    ...
    def test_temperature_matches_domain(self):
        profile = _make_profile(
            domain="creative", agent_role="cmo", complexity="simple", requires_reasoning=False
        )
        state = _make_state()
        config = select_model(profile, state)
