...
class _SimpleAgent(AgentBase):
    ...
    def plan(self, input_data: str) -> list[Task]:
        return [
            Task(id=f"t{i}", description=input_data, input={"i": i}) for i in range(int(input_data))
        ]
...
class _CountingAgent(AgentBase):
    ...
    def execute(self, task: Task) -> Result:
        ...
        return Result(
            ...
            error=None if success else "not yet",
        )
...
class TestInitSubclassWarning:
    ...
    def test_no_warning_for_abstract_subclass(self):
        ...
        assert not any("does not implement" in str(w.message) for w in caught)
