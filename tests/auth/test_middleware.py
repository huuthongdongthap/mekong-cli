...
class TestSessionMiddlewareProductionAuth:
    ...
    def test_valid_token_authenticates_user(self):
        ...
        sm.decode_token = MagicMock(
            return_value=(
                True,
                {"sub": str(mock_user.id), "email": mock_user.email, "role": "member"},
                None,
            )
        )

        with (
            patch("src.auth.middleware.AuthConfig") as MockConfig,
...
class TestOptionalAuthMiddleware:
    ...
    def test_valid_token_populates_user_state(self):
        ...
        sm.decode_token = MagicMock(
            return_value=(
                True,
                {"sub": str(mock_user.id), "email": mock_user.email, "role": "admin"},
                None,
            )
        )
