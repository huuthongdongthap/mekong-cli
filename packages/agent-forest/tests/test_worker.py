...
def test_process_one_updates_status_on_success(fake_redis, settings):
    ...
    outcome = process_one(fake_redis, settings, key="job:usr_founder1:j1", executor=stub)
...
def test_process_one_records_failure(fake_redis, settings):
    ...
    q.enqueue_job(fake_redis, job_id="j2", user_id="usr_founder2", prompt="x", webhook_url=None)

    def stub(_prompt: str, _sandbox: str, *, max_rounds: int = 1) -> JobOutcome:
        return JobOutcome(status="failed", error="boom")

    outcome = process_one(fake_redis, settings, key="job:usr_founder2:j2", executor=stub)
...
def test_process_one_emits_signal_with_outcome(fake_redis, settings, monkeypatch):
    ...
    q.enqueue_job(fake_redis, job_id="j9", user_id="usr_sig", prompt="x", webhook_url=None)

    def stub(_p, _s, *, max_rounds: int = 1) -> JobOutcome:
        return JobOutcome(status="completed", result="ok")

    worker_main.process_one(fake_redis, settings, key="job:usr_sig:j9", executor=stub)
...
def test_process_one_emits_bad_signal_with_error(fake_redis, settings, monkeypatch):
    ...
    monkeypatch.setattr(
        worker_main,
        "emit_signal",
        lambda status, uid, jid, *, error=None, **_kw: calls.append((status, uid, jid, error))
        or True,
    )

    q.enqueue_job(fake_redis, job_id="j10", user_id="usr_sig", prompt="x", webhook_url=None)

    def stub(_p, _s, *, max_rounds: int = 1) -> JobOutcome:
        return JobOutcome(status="failed", error="explode")

    worker_main.process_one(fake_redis, settings, key="job:usr_sig:j10", executor=stub)
...
def test_process_one_increments_completed_counter(fake_redis, settings):
    ...
    q.enqueue_job(fake_redis, job_id="cnt1", user_id="usr_founder1", prompt="x", webhook_url=None)
...
def test_process_one_increments_failed_counter(fake_redis, settings):
    ...
    q.enqueue_job(fake_redis, job_id="cnt2", user_id="usr_founder2", prompt="x", webhook_url=None)
...
def test_process_one_counter_graceful_on_redis_error(fake_redis, settings, monkeypatch):
    ...
    q.enqueue_job(fake_redis, job_id="cnt3", user_id="usr_founder1", prompt="x", webhook_url=None)
...
def test_run_loop_honors_max_iterations(fake_redis, settings, monkeypatch):
    ...
    iters = worker_main.run_loop(settings, max_iterations=1, executor=lambda *_a, **_k: None)
    assert iters == 1
