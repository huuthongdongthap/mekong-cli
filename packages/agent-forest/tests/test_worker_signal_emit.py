...
def test_emit_returns_false_on_non_2xx():
    respx.post("http://mekongd.test:8765/v1/signals").mock(return_value=httpx.Response(500))
    ok = signals.emit("completed", "u", "j", base_url="http://mekongd.test:8765")
    assert ok is False


@respx.mock
def test_emit_swallows_connection_error():
    respx.post("http://dead.test:8765/v1/signals").mock(side_effect=httpx.ConnectError("refused"))
    ok = signals.emit("completed", "u", "j", base_url="http://dead.test:8765")
...
def test_emit_user_feedback_posts_rating_as_kind():
    ...
    ok = signals.emit_user_feedback("good", "usr_a", "job_7", base_url="http://m.test:8765")
    assert ok is True
    body = _last_body(respx.calls)
...
def test_emit_user_feedback_truncates_long_note():
    ...
    signals.emit_user_feedback("bad", "u", "j", note="X" * 500, base_url="http://m.test:8765")
    body = _last_body(respx.calls)
