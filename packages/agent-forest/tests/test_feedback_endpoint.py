...
def test_feedback_404_when_job_not_owned(authed_client: TestClient):
    res = authed_client.post("/task/nonexistent/feedback", json={"rating": "good"})
    assert res.status_code == 404


def test_feedback_accepts_good_rating_and_forwards(authed_client: TestClient, monkeypatch):
    calls: list[tuple] = []
...
def test_feedback_records_false_forwarded_when_mekongd_down(authed_client: TestClient, monkeypatch):
    monkeypatch.setattr(routes_task, "emit_user_feedback", lambda *a, **kw: False)
    jid = _enqueue(authed_client)
...
def test_feedback_rejects_invalid_rating(authed_client: TestClient):
    jid = _enqueue(authed_client)
    res = authed_client.post(f"/task/{jid}/feedback", json={"rating": "meh"})
