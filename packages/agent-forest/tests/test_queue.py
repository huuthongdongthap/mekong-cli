...
def test_enqueue_persists_record(fake_redis):
    ...
    record = q.enqueue_job(fake_redis, job_id="j1", user_id="u1", prompt="hi", webhook_url=None)
...
def test_update_status(fake_redis):
    ...
    q.enqueue_job(fake_redis, job_id="j1", user_id="u1", prompt="x", webhook_url=None)
    q.update_job_status(fake_redis, "u1", "j1", status="completed", result="ok", error=None)
