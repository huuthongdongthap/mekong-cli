...
def create_task(
    ...
    return TaskEnqueued(job_id=job_id, status=record["status"], created_at=record["created_at"])
...
def submit_feedback(
    ...
    if not data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Job not found")
    forwarded = emit_user_feedback(body.rating, user.user_id, job_id, note=body.note)
