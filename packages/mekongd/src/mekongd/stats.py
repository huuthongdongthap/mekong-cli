...
def aggregate_signals(db_path: Path) -> dict[str, int]:
    ...
    if not db_path.exists():
        return {}
    with _connect(db_path) as conn:
        rows = conn.execute("SELECT kind, COUNT(*) FROM signals GROUP BY kind").fetchall()
...
def aggregate_user_signals(db_path: Path) -> dict[str, int]:
    ...
    with _connect(db_path) as conn:
        rows = conn.execute(
            "SELECT kind, COUNT(*) FROM signals " "WHERE note LIKE ? GROUP BY kind",
            ("%#user%",),
        ).fetchall()
    return {kind: int(count) for kind, count in rows}
...
def list_recent_signals(db_path: Path, limit: int = 20) -> list[dict]:
    ...
    return [
        {"ts": ts, "kind": kind, "note": note, "model": model} for ts, kind, note, model in rows
    ]
