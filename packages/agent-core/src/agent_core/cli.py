...
def report_cmd(
    ...
    hours: int = typer.Option(0, "--hours", help="Limit breakdown to last N hours (0 = all time)."),
    notes: int = typer.Option(
        0, "--notes", help="Append last N signal notes to output (0 = none)."
    ),
    cost: bool = typer.Option(False, "--cost", help="Append cloud cost breakdown by model."),
    ...
    if source not in {"all", "auto", "user"}:
        typer.echo(f"--source must be one of all|auto|user, got {source!r}", err=True)
...
def orchestrate_cmd(
    ...
    for round_ in session.rounds:
        ...
        typer.echo(
            f"Analyst: trend={round_.analyst['trend']} " f"summary={round_.analyst['summary']}"
        )
...
def _print_report(report) -> None:
    ...
    typer.echo(f"--- Tester ({report.test['status'].upper()}) ---\n" f"{report.test['summary']}")
...
def history_cmd(
    ...
    as_json: bool = typer.Option(False, "--json", help="In ra JSON thô thay vì bảng."),
) -> None:
...
def doctor_cmd(
    ...
    mekongd_status = (
        _ping(mekongd_url or os.environ.get("MEKONGD_URL") or "")
        if (mekongd_url or os.environ.get("MEKONGD_URL"))
        else "(skipped — no URL)"
    )
...
def eval_cmd(  # noqa: B008
    ...
    as_json: bool = typer.Option(
        False, "--json", help="Emit structured JSON output for CI."
    ),  # noqa: B008
...
def experiment_cmd(
    ...
    if json_out:
        typer.echo(json.dumps({"user": user, "experiment": name, "variant": assignment}))
    else:
