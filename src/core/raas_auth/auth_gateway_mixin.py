...
class AuthGatewayMixin:
    ...
    def _call_gateway_validation(self, token: str, endpoint: str) -> AuthResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return AuthResult(
                    valid=False, error="Invalid credentials", error_code="invalid_credentials"
                )
    ...
    def validate_credentials(
        ...
        if not token:
            return AuthResult(
                valid=False, error="No credentials provided", error_code="missing_credentials"
            )
    ...
    def verify_gateway(self, token: Optional[str] = None) -> GatewayVerifyResult:
        ...
        if not token:
            return GatewayVerifyResult(
                valid=False, error="No credentials provided", requires_auth=True
            )
        ...
        try:
            ...
            response = _get_requests().get(
                f"{self.gateway_url}{VERIFY_ENDPOINT}", headers=headers, timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                return GatewayVerifyResult(
                    valid=False, error="Invalid or expired credentials", requires_auth=True
                )
            elif response.status_code == 403:
                return GatewayVerifyResult(
                    valid=False,
            ...
            return GatewayVerifyResult(
                valid=False, error=f"Gateway unreachable: {str(e)}", gateway_status="unreachable"
            )
    ...
    def sync_to_dashboard(self) -> Dict[str, Any]:
        ...
        if not session.authenticated:
            return {
                ...
                "dashboard_url": "https://www.mekongmind.com/dashboard",
            }
