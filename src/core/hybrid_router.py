...
async def route_and_execute(
    ...
    task_tier = (
        "mechanical"
        if profile.complexity == "simple" and not profile.requires_reasoning
        else "integration"
    )
    ...
    if is_multi_agent:
        ...
        for agent_role in agent_roles:
            ...
            if exec_result.success:
                ...
                flow.add(
                    agent_role, exec_result.error, status="BLOCKED", concerns=exec_result.error
                )
    ...
    logger.info(
        ...
        confirm_result.charged,
        confirm_result.refunded,
    )
