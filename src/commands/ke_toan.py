...
class InvoiceItem:
    ...
    def vat_amount(self) -> Decimal:
        return (self.subtotal * VAT_RATES[self.vat_rate]).quantize(
...
class Invoice:
    ...
    def to_journal_entry(self) -> dict:
        ...
        return {
            ...
            "entries": [
                {
                    "account": VAS_ACCOUNTS["receivable"],
                    "debit": str(self.total_amount),
                    "credit": "0",
                    "note": "Phải thu khách hàng",
                },
                {
                    "account": VAS_ACCOUNTS["revenue"],
                    "debit": "0",
                    "credit": str(self.subtotal),
                    "note": "Doanh thu bán hàng",
                },
                {
                    "account": VAS_ACCOUNTS["vat_output"],
                    "debit": "0",
                    "credit": str(self.total_vat),
    ...
    def to_summary(self) -> str:
        ...
        for item in self.items:
            lines.append(
                f"  - {item.description}: {self.format_vnd(item.unit_price)} × {item.quantity} = {self.format_vnd(item.subtotal)}"
            )
        lines += [
            "",
