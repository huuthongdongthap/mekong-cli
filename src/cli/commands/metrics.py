...
_POSTHOG_URL = os.environ.get("SIGNALS_POSTHOG_URL", "https://posthog.m1max.cashclaw.cc")
...
def _show_metrics(days: int = 7, agent: str = "all") -> None:
    ...
    posthog_status = "[green]enabled[/green]" if _POSTHOG_ENABLED else "[dim]deferred[/dim]"
    ...
    if result.total_missions == 0:
        console.print(f"\n[dim]No missions in last {days} days. " "Run a mission then retry.[/dim]")
        return
