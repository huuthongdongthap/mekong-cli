...
def status() -> None:
    ...
    table.add_row(
        "Last Check",
        (
            cache_status.get("checked_at", "Never")
            if cache_status.get("checked_at") != "0001-01-01T00:00:00+00:00"
            else "Never"
        ),
    )
    table.add_row(
    ...
    if critical_update:
        ...
        table.add_row(
            "Action Required", "[bold red]Run 'mekong update install' immediately[/bold red]"
        )
    else:
