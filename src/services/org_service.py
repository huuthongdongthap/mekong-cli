...
def create_org(display_name: str, founder_email: str) -> dict[str, Any]:
    ...
    raw_payload = json.dumps({"display_name": display_name, "founder_email": founder_email})
...
def remove_member(org_id: str, user_id: str, actor_email: str) -> None:
    ...
    try:
        # Verify org exists
        org_row = conn.execute("SELECT 1 FROM orgs WHERE org_id = ?", (org_id,)).fetchone()
        if org_row is None:
            raise OrgNotFoundError(f"Org '{org_id}' not found")
        ...
        if member_row["scope"] == "org_admin":
            ...
            if admin_count <= 1:
                raise LastAdminError(f"Cannot remove the last org_admin from '{org_id}'")
...
_VALID_INVITE_SCOPES: frozenset[str] = frozenset({"org_admin", "cs", "marketing", "readonly"})
...
def create_invite(
    ...
    if scope not in _VALID_INVITE_SCOPES:
        raise InvalidInviteScopeError(f"Scope '{scope}' not in {sorted(_VALID_INVITE_SCOPES)}")
    ...
    try:
        ...
        if member_row is not None:
            raise AlreadyMemberError(f"'{invitee_email}' is already a member of org '{org_id}'")
...
def accept_invite(invite_id: str, joiner_email: str) -> dict[str, Any]:
    ...
    try:
        ...
        if row["invitee_email"].lower() != joiner_email.lower():
            conn.rollback()
            raise InviteEmailMismatchError("Email does not match invite recipient")
