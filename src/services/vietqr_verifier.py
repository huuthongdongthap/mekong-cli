...
def get_verifier() -> VietQRVerifier:
    ...
    if provider == "sepay":
        ...
        if not secret:
            raise RuntimeError("MEKONG_VIETQR_WEBHOOK_SECRET not set — VietQR webhook disabled")
        return SepayVerifier(secret)
    if provider == "insecure_dev":
        return NoOpVerifier()
    raise RuntimeError(
        f"Unknown MEKONG_VIETQR_PROVIDER={provider!r} — " "supported: sepay, insecure_dev"
    )
