...
def check_rate_limit(email: str, window_hours: int = 1, max_count: int = 5) -> bool:
    ...
    """
    window_start = (datetime.now(timezone.utc) - timedelta(hours=window_hours)).strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    ) + "Z"
    ...
    try:
        row = conn.execute(
            "SELECT COUNT(*) AS cnt FROM magic_link_tokens " "WHERE email = ? AND created_at > ?",
            (email, window_start),
        ).fetchone()
...
def purge_expired(grace_hours: int = 24) -> int:
    ...
    """
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=grace_hours)).strftime(
        "%Y-%m-%dT%H:%M:%S.%f"
    ) + "Z"
