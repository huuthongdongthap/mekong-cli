...
def network_optimize(
    ...
    if status_only:
        ...
        warp = report["warp"]
        console.print(
            f"WARP: {'[green]✅ Connected[/green]' if warp['connected'] else '[red]❌ Disconnected[/red]'}"
        )

        ts = report["tailscale"]
        console.print(
            f"Tailscale: {'[green]✅ Online[/green]' if ts.get('online') else '[red]❌ Offline[/red]'}"
        )
        console.print(
            f"Exit Node: {'[green]✅ Active[/green]' if ts.get('exit_node_active') else '[yellow]❌ Disabled[/yellow]'}"
        )
...
def health_check():
    ...
    console.print(
        f"  {'[green]✅[/green]' if is_python_ok else '[red]❌[/red]'} Python 3 Environment"
    )
    all_ok &= is_python_ok
    ...
    for mod in modules:
        is_mod_ok = (
            Path(mod).exists() and Path(f"{mod}/__init__.py").exists()
            if mod != "backend"
            else Path(mod).exists()
        )
    ...
    console.print(
        f"  {'[green]✅[/green]' if is_prod_ok else '[yellow]⚠️[/yellow]'} Products ({len(products)} found)"
    )
