...
export default function NewSchoolPage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<SchoolFormData>({
  ...
  async function onSubmit(data: SchoolFormData) {
    setError("")
    try {
      const res = await api.post<{ data: { id: string } }>("/schools", data)
      router.push(`/schools/${res.data.id}`)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Lỗi tạo trường")
    }
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold">Thêm trường đối tác</h2>
        <Link href="/schools" className="text-sm text-muted-foreground hover:underline">Quay lại</Link>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-lg border p-6">
        ...
        <div className="flex justify-end gap-3 pt-4">
          <Link href="/schools" className="inline-flex items-center justify-center rounded-md border px-4 py-2 text-sm hover:bg-accent">Hủy</Link>
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? "Đang tạo..." : "Tạo trường"}
          </Button>
        </div>
      </form>
    </div>
