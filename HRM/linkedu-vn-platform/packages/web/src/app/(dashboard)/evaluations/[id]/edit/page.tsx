...
export default function EditEvaluationPage({ params }: { params: { id: string } }) {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<EvaluationFormData>({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await api.get<{ data: Partial<EvaluationFormData> & { totalScore?: number | string; maxScore?: number | string; evaluatedAt?: string } }>(`/evaluations/${params.id}`)
        const e = res.data
        reset({
          enrollmentId: e.enrollmentId || "", learnerId: e.learnerId || "",
          evaluatorId: e.evaluatorId || "",
          evaluationType: e.evaluationType || "mid_term",
          totalScore: e.totalScore?.toString() || "",
          maxScore: e.maxScore?.toString() || "",
          feedback: e.feedback || "", strengths: e.strengths || "",
          improvements: e.improvements || "",
          evaluatedAt: e.evaluatedAt?.slice(0, 10) || "",
        })
      } catch (err) {
  ...
  async function onSubmit(data: EvaluationFormData) {
    ...
    try {
      ...
      await api.put(`/evaluations/${params.id}`, payload)
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold">Sửa đánh giá</h2>
        <Link href={`/evaluations/${params.id}`} className="text-sm text-muted-foreground hover:underline">Quay lại</Link>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 rounded-lg border p-6">
        ...
        <div className="flex justify-end gap-3 pt-4">
          <Link href={`/evaluations/${params.id}`} className="inline-flex items-center justify-center rounded-md border px-4 py-2 text-sm hover:bg-muted/50">Hủy</Link>
