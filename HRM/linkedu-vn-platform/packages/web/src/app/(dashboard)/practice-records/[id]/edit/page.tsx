...
export default function EditPracticeRecordPage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState("")
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<PracticeRecordFormData>({
    ...
    const load = async () => {
      try {
        const res = await api.get<{ data: {
          enrollmentId?: string; learnerId?: string; enterpriseId?: number | string
          practiceDate?: string; activities?: string | null
          hoursWorked?: number | string; supervisorName?: string | null
          skillsDemonstrated?: string[]; feedback?: string | null; rating?: number | string | null
        } }>(`/practice-records/${params.id}`)
        const r = res.data
        reset({
          enrollmentId: r.enrollmentId || "", learnerId: r.learnerId || "",
          enterpriseId: r.enterpriseId?.toString() || "",
          practiceDate: r.practiceDate?.slice(0, 10) || "",
  ...
  async function onSubmit(data: PracticeRecordFormData) {
    ...
    try {
      ...
      await api.put(`/practice-records/${params.id}`, payload)
      router.push(`/practice-records/${params.id}`)
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold">Sửa bản ghi thực tập</h2>
        <Link href={`/practice-records/${params.id}`} className="text-sm text-muted-foreground hover:underline">Quay lại</Link>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 rounded-lg border p-6">
        ...
        <div className="flex justify-end gap-3 pt-4">
          <Link href={`/practice-records/${params.id}`} className="inline-flex items-center justify-center rounded-md border px-4 py-2 text-sm hover:bg-muted/50">Hủy</Link>
